...
fn to_json(bands: &[Band]) -> String {
	let features: Vec<geojson::Feature> = bands.iter().map(|band| band.to_geojson()).collect();
...
pub fn contours(data: JsValue) -> Result<String, JsValue> {
	...
	// Transpose data vec
	let data_transpose = data.clone().into_iter().enumerate().map(|(i, _)| data.iter().map(|row| row[i]).collect()).collect::<Vec<Vec<f64>>>();

	// Flatten data vec
	let data_flat: Vec<f64> = data_transpose.clone().into_iter().flatten().collect();

	let len_x = data_transpose.len();
	let len_y = data_transpose.get(0).ok_or(miette!("Image has no width lol")).map_err(|e| e.to_string())?.len();
